...
const { checkUrlCanLoad, judgeIsStrictLTPPUrl, judgeIsLTPPUrl, sleep, judgeIsValidUrl } = require('./public.js');
...
function creatLoadingWindow () {
    ...
    loading_window.webContents.setFrameRate(240);
    creatChildWin(loading_window);
    // 资源加载放最后
    loading_window.loadFile(splashscreen_path);
    loading_window.once('ready-to-show', () => {
        try {
            // 资源准备就绪后展示
            loading_window.show();
            loading_window.setAlwaysOnTop(true);
            setTimeout(() => {
                loading_window.setAlwaysOnTop(false);
            }, 0);
        } catch (err) {
            app.quit();
        }
 ...
 */
async function createWindow (url = app_url, param_new_window_config = {}, param_web_preferences_config = {}) {
    ...
    if (!res) {
        ...
        loading_window.hide();
        await showMessageBox(system_info_title, network_error_msg, [is_strict_ltpp_url ? ltpp_main_btn_error_msg : ltpp_other_btn_error_msg]);
    ...
    main_window.webContents.setFrameRate(240);
...
function creatChildWin (main_window) {
    ...
    main_window.webContents.setWindowOpenHandler((details) => {
        ...
        if (!url || !judgeIsValidUrl(url)) {
            showMessageBox(system_info_title, url_illegal_error_msg, [ltpp_other_btn_error_msg]);
            return { action: 'deny' };
        }
        const is_ltpp_url = judgeIsLTPPUrl(url);
        if (!is_ltpp_url) {
            showMessageBox(system_info_title, url_will_to_outer_msg, [ltpp_other_btn_confirm_msg, ltpp_other_btn_cancel_msg]).then((click_idx) => {
                // 传入的按钮数组，下标为0的为确认
                if (click_idx === 0) {
        ...
        checkUrlCanLoad(url).then((res) => {
            if (!res) {
                showMessageBox(system_info_title, network_error_msg, [ltpp_other_btn_error_msg]);
                return { action: 'deny' };
            }
        });
