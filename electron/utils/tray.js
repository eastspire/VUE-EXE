...
function creatTray (main_window) {
    ...
    for (const key in language_map) {
        if (Object.hasOwnProperty.call(language_map, key)) {
            ...
            web_ide_submenu.push({
                label: `${key}编辑器`, icon: map.icon, click: () => {
                    createWindow(`${ltpp_web_ide_url}${map.value}`, {
                        resizable: true,
                        movable: true,
                        frame: true,
                        transparent: false,
    ...
    const contextMenu = Menu.buildFromTemplate([
        {
            label: '我在线上', icon: online_icon, click: () => {
            }
        },
        {
            label: '应用文档', icon: docs_icon, click: () => {
                createWindow(docs_url, {
                    resizable: true,
                    movable: true,
                    frame: true,
                    transparent: false,
        ...
        {
            label: '代码编辑器', icon: web_ide_icon, submenu: web_ide_submenu
        },
        {
        ...
        {
            label: '显示窗口', icon: maximize_icon, click: () => {
                if (main_window) {
        ...
        {
            label: '隐藏窗口', icon: minimize_icon, click: () => {
                if (main_window) {
                    main_window.hide();
                }
            }
        ...
        {
            label: '重启应用', icon: restart_icon, click: () => {
                app.relaunch();
                app.exit(0);
            }
        },
